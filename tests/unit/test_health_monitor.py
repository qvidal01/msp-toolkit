...
from msp_toolkit.core.models import HealthCheckStatus, HealthCheckType
...
class TestHealthMonitor:
    ...
    def test_run_checks_accepts_string_types(self, test_config):
        """Allow string inputs to be coerced into enums."""
        monitor = HealthMonitor(test_config)

        results = monitor.run_checks("test-client", check_types=["cpu", "disk"])

        assert {r.check_type for r in results} == {HealthCheckType.CPU, HealthCheckType.DISK}

    def test_run_checks_rejects_unknown_type(self, test_config):
        """Invalid check types should raise a ValidationError."""
        monitor = HealthMonitor(test_config)

        with pytest.raises(ValidationError):
            monitor.run_checks("test-client", check_types=["cpu", "invalid-check"])
